...
<div class="max-w-2xl mx-auto space-y-5 animate-fadeSlideIn">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.25rem;">
        <div>
            <h2 style="font-size:1.1rem;font-weight:800;color:var(--t1);letter-spacing:-0.01em;">Edit Shift Kerja</h2>
            <p style="font-size:0.78rem;color:var(--t4);margin-top:0.25rem;">Perbarui jam operasional kerja, toleransi keterlambatan, dan pengaturan shift karyawan</p>
        </div>
    ...
    <div class="card">
        <form method="POST" action="{{ route('master.shifts.update', $shift->id) }}" style="display:flex;flex-direction:column;gap:1.25rem;">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;gap:0.5rem;align-items:center;">
                        <input type="color" name="color" id="color" class="form-control"
                            ...
                            value="{{ old('color', $shift->color ?? '#10b981') }}">
                        <span id="colorLabel" style="font-size:0.72rem;color:var(--t4);font-family:'JetBrains Mono',monospace;">{{ old('color', $shift->color ?? '#10b981') }}</span>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;align-items:center;gap:0.4rem;">
                        <input type="number" name="early_out_tolerance_minutes" id="early_out_tolerance_minutes" class="form-control"
                            ...
                            value="{{ old('early_out_tolerance_minutes', $shift->early_out_tolerance_minutes) }}">
                        <span style="font-size:0.72rem;color:var(--t4);white-space:nowrap;">menit</span>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select name="is_overnight" id="is_overnight" class="form-control">
                        <option value="0" {{ old('is_overnight', $shift->is_overnight) == 0 ? 'selected' : '' }}>Tidak (hari sama)</option>
                        <option value="1" {{ old('is_overnight', $shift->is_overnight) == 1 ? 'selected' : '' }}>Ya (melewati tengah malam)</option>
                    </select>
