...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-800" style="color: var(--text-main);">Edit Lokasi Kantor</h2>
            <p class="text-xs text-slate-500">Perbarui koordinat geofencing dan jangkauan absensi kantor</p>
        </div>
    ...
    <div class="card">
        <form method="POST" action="{{ route('master.locations.update', $location->id) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control" placeholder="Contoh: Kantor Pusat Jakarta" value="{{ old('name', $location->name) }}" required>
                @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="any" name="latitude" id="latitude" class="form-control" placeholder="Contoh: -6.2088" value="{{ old('latitude', $location->latitude) }}" required>
                    @error('latitude')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="any" name="longitude" id="longitude" class="form-control" placeholder="Contoh: 106.8456" value="{{ old('longitude', $location->longitude) }}" required>
                    @error('longitude')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="radius_meters" id="radius_meters" class="form-control" placeholder="Contoh: 100" min="10" max="5000" value="{{ old('radius_meters', $location->radius_meters) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea name="address" id="address" rows="3" class="form-control" placeholder="Tuliskan alamat fisik lengkap kantor...">{{ old('address', $location->address) }}</textarea>
                @error('address')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-700 text-xs flex gap-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Map centered on existing coordinates
