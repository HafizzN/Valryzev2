...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="card">
        <div style="margin-bottom: 1.5rem; padding-bottom: 0.75rem; border-bottom: 1px solid rgba(255,255,255,0.06);">
            <h3 style="font-size: 1rem; font-weight: 600; color: #e2e8f0;">Informasi Profil</h3>
            <p style="font-size: 0.75rem; color: #64748b; margin-top: 0.25rem;">Perbarui nama dan alamat email akun Anda.</p>
        </div>
        ...
        <form method="post" action="{{ route('profile.update') }}" class="space-y-4" enctype="multipart/form-data" id="profile-form">
            ...
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.5rem;">
                @if($user->photo)
                    <img id="profile-preview-img" src="{{ $user->photo_url }}" alt="{{ $user->name }}" style="width: 72px; height: 72px; border-radius: 9999px; object-fit: cover; border: 2px solid #16a34a;">
                @else
                    <div id="profile-preview-initials" style="width: 72px; height: 72px; border-radius: 9999px; background: linear-gradient(135deg, var(--primary), var(--primary-light)); color: white; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; font-weight: 700; border: 2px solid #16a34a; text-transform: uppercase;">
                        {{ $user->initials }}
                    </div>
                    <img id="profile-preview-img" style="display: none; width: 72px; height: 72px; border-radius: 9999px; object-fit: cover; border: 2px solid #16a34a;">
                @endif
                <div>
                    <div style="font-size: 0.82rem; font-weight: 600; color: #f1f5f9;">Foto Profil</div>
                    <div style="font-size: 0.72rem; color: #94a3b8;">Format JPG atau PNG, maks. 2 MB</div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input id="photo" name="photo" type="file" class="form-control text-xs text-slate-600
                    file:mr-4 file:py-2 file:px-4
                    file:rounded-md file:border-0
                    file:text-xs file:font-semibold
                    file:bg-emerald-950 file:text-emerald-400
                    hover:file:bg-emerald-900" style="padding: 0.5rem; height: auto;">
                @if($errors->has('photo'))
    ...
    <div class="card">
        <div style="margin-bottom: 1.5rem; padding-bottom: 0.75rem; border-bottom: 1px solid rgba(255,255,255,0.06);">
            <h3 style="font-size: 1rem; font-weight: 600; color: #e2e8f0;">Perbarui Kata Sandi</h3>
            <p style="font-size: 0.75rem; color: #64748b; margin-top: 0.25rem;">Pastikan akun Anda menggunakan kata sandi yang kuat dan acak untuk menjaga keamanan.</p>
        </div>
